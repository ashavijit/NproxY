...
np_status_t tls_ctx_create(np_tls_ctx_t *tc, const np_server_config_t *cfg) {
  ...
  SSL_CTX_set_options(tc->ctx, SSL_OP_NO_SSLv3 | SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1 |
  ...
  if (SSL_CTX_use_certificate_file(tc->ctx, cfg->tls.cert_file, SSL_FILETYPE_PEM) <= 0) {
  ...
  if (SSL_CTX_use_PrivateKey_file(tc->ctx, cfg->tls.key_file, SSL_FILETYPE_PEM) <= 0) {
