...
static void handle_write(conn_t *conn, client_ctx_t *cctx) {
  isize n;
  do {
...
static void handle_read(conn_t *conn, client_ctx_t *cctx) {
  worker_state_t *ws = cctx->ws;
  isize n;
  do {
  ...
  /* For non-proxy responses the response is already in wbuf.
   * With edge-triggered epoll EV_WRITE only fires on state transition,
   * so try to flush immediately. If unsent data remains, arm EV_WRITE. */
  if (conn->state == CONN_PROXYING) {
    /* proxy_handle already registered the upstream fd in epoll;
     * the response will arrive via proxy_on_upstream_event. */
    return;
  }

  /* Flush synchronously; if the socket is full, re-arm for EV_WRITE. */
  isize sent;
  do {
...
static void on_accept_event(int fd, u32 events, void *arg) {
  ...
  for (;;) {
    ...
    conn_t *conn = conn_create(cfd, &peer, ws->loop);
    if (!conn) {
      close(cfd);
      continue;
    }
