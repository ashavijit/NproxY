...
static void build_proxy_request(conn_t *conn, http_request_t *req) {
  ...
  int n = snprintf(buf, sizeof(buf),
                   ...
                   STR_ARG(request_header(req, STR("Host"))), req->remote_ip,
                   req->remote_ip);
  ...
  for (int i = 0; i < req->header_count; i++) {
    ...
    if (str_ieq(name, STR("Host")) || str_ieq(name, STR("Connection")))
      continue;
    int hn =
        snprintf(buf + n, sizeof(buf) - (usize)n, STR_FMT ": " STR_FMT "\r\n",
                 STR_ARG(name), STR_ARG(req->headers[i].value));
    if (hn > 0)
      n += hn;
  }
  ...
  if (req->body.len > 0 &&
      buf_writable(&conn->upstream_wbuf) >= req->body.len) {
...
void proxy_on_upstream_event(int fd, u32 events, void *arg) {
  ...
  if (events & EV_WRITE) {
    ...
    if (buf_readable(&conn->upstream_wbuf) == 0) {
      event_loop_mod(conn->loop, fd, EV_READ | EV_HUP | EV_EDGE,
  ...
  if (events & EV_READ) {
    ...
    if (n == NP_ERR_CLOSED) {
      event_loop_del(conn->loop, fd);
      conn->state =
          req_keep_alive_check(conn) ? CONN_READING_REQUEST : CONN_CLOSING;
    }
  }
}
...
void proxy_handle(conn_t *conn, http_request_t *req, handler_ctx_t *ctx) {
  ...
  event_loop_add(conn->loop, ufd, EV_WRITE | EV_READ | EV_HUP | EV_EDGE,
                 proxy_on_upstream_event, conn);
}
